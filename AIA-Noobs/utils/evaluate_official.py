...
import numpy as np
import pandas as pd
from utils.utils_official import TqdmLoggingHandler, validate_prediction_dir
from utils.metrics_official import mNSD, mIoU, accuracy, f1k
from tqdm import tqdm


def eval_official(ref_dir, prediction_dir, ignore_segmentation=False, ignore_actions=False):
    print('ignore_segmentation: ', ignore_segmentation, 'ignore_actions: ', ignore_actions)
    logging.basicConfig(level=logging.INFO,
                        handlers=[logging.FileHandler(Path(prediction_dir) / "evaluation.log",
                                                      encoding='utf-8'),
                                  TqdmLoggingHandler()])

    try:
        ref_video_dirs, pred_video_dirs = validate_prediction_dir(ref_dir,
                                                                  prediction_dir,
                                                                  actions=not ignore_actions,
                                                                  segmentation=not ignore_segmentation)
    ...
    for ref_dir, pred_dir in tqdm(zip(ref_video_dirs, pred_video_dirs),
        ...
        if not ignore_segmentation:
            ...
            results['seg_mNSD'].append(
                np.array(mNSD(pred_dir, ref_dir, n_classes=9, channel_tau=[NSD_tau] * 9)).mean(axis=1).mean().item())
    ...
    df.to_csv(Path(prediction_dir) / 'per_video_results.csv', index=False)
    m_df.to_csv(Path(prediction_dir) / 'final_results.csv')
