...
def validate_prediction_dir(test_dir, prediction_dir, segmentation=True, actions=True):
    print('segmentation: ', segmentation, 'actions: ', actions)
    ...
    for r_v, p_v in zip(ref_video_dirs, pred_video_dirs):
        if r_v.name != p_v.name:
            raise FileNotFoundError(
                f'video directory {p_v.name} is not present in the test directory or {r_v.name} is not present in the prediction directory')
    ...
    for r_v, p_v in zip(ref_video_dirs, pred_video_dirs):
        ...
        if segmentation:
            ...
            if len(list((p_v / 'segmentation').iterdir())) != len_ref_seg:
                ...
                print(unique)
                raise FileNotFoundError(
                    f"segmentation file validation failed. {r_v} contained {len_ref_seg} files while {p_v} contained {len(list((p_v / 'segmentation').iterdir()))}")
        ...
        if actions:
            ...
            if len_pred_action != len_ref_action:
                raise FileNotFoundError(
                    f"segmentation file validation failed. {(r_v / 'action_discrete.txt')} contained {len_ref_action} lines while {(p_v / 'action_discrete.txt')} contained {len_pred_action} lines")
            ...
            if not np.all(ref_action[:, 0] == pred_action[:, 0]):
                raise ValueError(
                    f"action file validation failed. {(p_v / 'action_discrete.txt')} contained predictions that do not correspond to {(r_v / 'action_discrete.txt')}")
